...
export const createPointMesh = (options) => {
  ...
  // 获取改坐标的三维向量（Vector3）并转换为单位向量（unit vector）
...
export const createLightPillar = (options) => {
  ...
  const material = new MeshBasicMaterial({
    ...
    transparent: true,
    side: DoubleSide,  // 前面FrontSide  背面：BackSide 双面：DoubleSide
  ...
  const meshNormal = new Vector3(0, 0, 1);
  // quaternion表示对象局部旋转的Quaternion（四元数）。控制旋转
...
export const createWaveMesh = (options) => {
  ...
  mesh.scale.set(size, size, size); //设置mesh大小
  mesh.userData['size'] = size; //自定义一个属性，表示mesh静态大小
